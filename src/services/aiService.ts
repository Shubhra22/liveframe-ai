...
if (apiKey && endpoint) {
  // Azure OpenAI Responses API endpoint: {endpoint}/responses?api-version=...
  client = new OpenAI({
    baseURL: endpoint,
    apiKey: apiKey,
    defaultHeaders: {
...
const MODEL = deploymentName || "gpt-5.1-codex-max";

/** Strip markdown code fences the model sometimes adds */
const stripFences = (text: string): string =>
  text.replace(/^```html\s*/i, '').replace(/^```\s*/, '').replace(/\s*```$/, '');
...
export const generateText = async (
  ...
  switch (action) {
    case AiActionType.REWRITE:
      prompt = `Rewrite the following text to be more engaging and clear, maintaining the original meaning: "${currentText}"`; break;
    case AiActionType.TRANSLATE:
      prompt = `Translate the following text to Spanish: "${currentText}"`; break;
    case AiActionType.SHORTER:
      prompt = `Shorten the following text while keeping the key information: "${currentText}"`; break;
    case AiActionType.LONGER:
      prompt = `Expand on the following text with more descriptive details: "${currentText}"`; break;
    case AiActionType.TONE_PROFESSIONAL:
      prompt = `Rewrite the following text to sound professional and corporate: "${currentText}"`; break;
    case AiActionType.TONE_CASUAL:
      prompt = `Rewrite the following text to sound casual and friendly: "${currentText}"`; break;
    default:
...
export const generateImage = async (_prompt: string): Promise<AiResponse> => {
  return { error: "Image generation not supported with this deployment. Consider using DALL-E API separately." };
};

export const convertHtmlToEmail = async (htmlCode: string): Promise<string> => {
  if (!client) throw new Error("AI service not initialized");

  const prompt = `You are an expert email developer.
...
${htmlCode}`;
  ...
  try {
    ...
    return stripFences(response.output_text || "");
  } catch (e: any) {
